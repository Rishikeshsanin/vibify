...
export async function GET(request: NextRequest) {
  ...
  if (!key) {
    return NextResponse.json({ error: 'Music search is not configured yet.' }, { status: 503 });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Music search failed.', detail }, { status: response.status });
    }
    ...
    return NextResponse.json(
      { error: 'Music search is temporarily unavailable.', detail: error instanceof Error ? error.message : 'Unknown upstream error' },
      { status: 502 }
    );
  }
}
