...
import { VibeModeLayer } from './VibeModeLayer';
...
export function RoomChat({ roomCode, uid, name, participants }: Props) {
  ...
  const suggestTrack = async (track: Track) => {
    ...
    try {
      ...
      if (existing) {
        await setVibeVote(roomCode, uid, track.videoId, true);
      } else {
        await suggestVibePick(roomCode, uid, name, track);
      }
  ...
  return (
    <>
      <VibeModeLayer roomCode={roomCode}/>
      ...
      {boardOpen && (
        <div className="vibe-board-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) setBoardOpen(false); }}>
          ...
          <VibePicks
            ...
            onSuggest={() => setSuggestSearchOpen(true)}
            onVote={pick => { void voteForPick(pick); }}
            onQueue={pick => { void queuePick(pick); }}
            onPlay={pick => { void playPick(pick); }}
            onPass={pick => { void passPick(pick); }}
          />
        ...
        <div className="vibe-suggest-search-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) setSuggestSearchOpen(false); }}>
          <section className="search-modal glass v2-search-modal vibe-suggest-search">
            <div className="search-head">
              <div><span className="eyebrow">PITCH A TRACK</span><h2>What should the room hear next?</h2></div>
              <button className="icon-button" onClick={() => setSuggestSearchOpen(false)}><X/></button>
            </div>
            <form className="search-box" onSubmit={searchSongs}>
              <Search/>
              <input autoFocus value={searchQuery} onChange={event => setSearchQuery(event.target.value)} placeholder="Search songs or artists…"/>
              <button type="submit" disabled={searching}>{searching ? <LoaderCircle className="spin"/> : 'Search'}</button>
            </form>
            {pickError && <p className="search-error">{pickError}</p>}
            <div className="results-list">
              {searchResults.length === 0 && !searching && (
                <div className="search-empty"><Music2/><b>Pitch your next song</b><span>It will go to Vibe Picks for the room to vote on.</span></div>
              )}
                ...
                return (
                  <div key={track.videoId} className="track-result track-result-v2">
                    ...
                    <div className="result-actions-v2 vibe-result-actions">
                      <button
                        className="vibe-result-suggest"
                        disabled={alreadyVoted || Boolean(pickBusyId)}
                        onClick={() => { void suggestTrack(track); }}
                      >
                        {pickBusyId === track.videoId ? <LoaderCircle className="spin" size={16}/> : <Flame size={16}/>} 
                        <span>{alreadyVoted ? 'Already vibed' : existing ? '+1 Vibe' : 'Suggest'}</span>
                      </button>
                    </div>
                  </div>
                );
